...
/**
 * Tests a calendar that can use a timezone.
 */
public class TestTimezoneCalendar {
  Event dentist;
  Event doctor;
  Event convention;
  Event classes;
  ...
  public void testCreateEventSeriesNTimes() {
    ...
    try {
      ...
      assertThat(e.getMessage(), is("Either end date or number of " +
  ...
  public void testEditEvent() {
    ...
    calendar.editEvent("end", "Holiday",
            "2025-12-20T09:00", "2025-12-31T09:00",
            "2025-12-25T09:00");
    ...
    calendar.createEventSeriesNTimes(classes, "MWF", 3);
    calendar.editEvent("location", "Classes", "2025-02-01T15:00",
            "2025-02-01T16:00", "Room 100");
  ...
  public void testUpdateTimes() {
    setup();
    // test timezone update for single day events
    calendar.updateTimes(TimeZone.getTimeZone("Europe/London"));
    assertEquals("• Dentist Appointment (2025-01-01 14:00 - 15:00)\n" +
                    "• Doctors Appointment (2025-01-01 17:00 - 18:00)\n",
            calendar.daySchedule("2025-01-01"));

    // test invalid timezone update
    try {
      calendar.updateTimes(TimeZone.getTimeZone("Invalid/Zone"));
    } catch (IllegalArgumentException e) {
      assertThat(e.getMessage(), is("Invalid timezone"));
    }

    // test updating multiple events across different dates
    calendar.updateTimes(TimeZone.getTimeZone("Asia/Tokyo"));
    assertEquals("• Dentist Appointment (2025-01-01 23:00 - 2025-01-02 00:00)\n" +
                    "• Doctors Appointment (2025-01-02 02:00 - 03:00)\n",
            calendar.rangeSchedule("2025-01-01T00:00", "2025-01-02T23:59"));
  }
}